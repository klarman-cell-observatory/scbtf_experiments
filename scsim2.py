#
# Adapted from https://github.com/dylkot/scsim2/tree/main
#
#
#
#
...
class scsim2:
    ...
    def simulate_counts(self):
        ...
        counts = np.random.poisson(lam=self.updatedmean)
        
        eta = [np.log(x) for x in self.geneparams.BaseGeneMean.values]
        drop_prob = [scsim2.logistic(e, x0=1*np.mean(eta), k=0) for e in eta]
        
        keep = 1 - np.random.binomial(1, drop_prob, size=counts.shape)
        
        counts = counts * keep
        
        self.counts = pd.DataFrame(counts, index=self.updatedmean.index, columns=self.updatedmean.columns)
